...
def apply_template_defaults(template_type):
    """Apply default values from a campaign template."""
...
def handle_chat_input(user_input, model_name, temperature, openai_api_key, google_api_key, output_format):
    """Handles user input and generates responses with real-time context updates."""
    try:
...
def main():
    ...
    with st.expander("📝 Campaign Details", expanded=True):
        ...
        specific_instructions = st.text_area("Additional instructions for the AI (e.g., keywords, goals, competitors).", key="specific_instructions_input")
